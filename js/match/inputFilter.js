...
function checkOneRule(string, lcString, exact, res, oRule, cntRec) {
    ...
    switch (oRule.type) {
        case fdevsta_monmove_1.IFModel.EnumRuleType.WORD:
            ...
            break;
        case fdevsta_monmove_1.IFModel.EnumRuleType.REGEXP:
            {
                if (debuglog.enabled) {
...
function analyzeString(sString, rules, words) {
    var cnt = 0;
    var fac = 1;
    var u = fdevsta_monmove_2.BreakDown.breakdownString(sString, Algol.MaxSpacesPerCombinedWord);
    if (debuglog.enabled) {
